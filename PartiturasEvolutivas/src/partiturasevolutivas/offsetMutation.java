 ...
 * @author Daniel Dyer
 */
public class offsetMutation implements EvolutionaryOperator<Nota>
{
     ...
     */
    public offsetMutation(NumberGenerator<Probability> mutationProbability)
    {
    ...
    public List<Nota> apply(List<Nota> partitura, Random rng)
    {
        ...
        {
            Nota nn=new Nota(p.getNote(),p.isSharp(),p.getScale(),p.getLength(),p.isHalf(),p.isRest());
            nuevaPartitura.add(nn);
        }
        if (getMutationProbability().nextValue().nextEvent(rng)){   
            Nota n=nuevaPartitura.get(0);
            nuevaPartitura.remove(0);
            nuevaPartitura.add(nuevaPartitura.size(), n);
        }
